...
import com.hrmpandjiadhi.databinding.ActivityAddNewFormLeaveStaffBinding
import com.hrmpandjiadhi.ui.viewmodel.leave.CreateLeaveViewModel
import com.hrmpandjiadhi.ui.viewmodel.leave.CreateLeaveViewModelFactory
import com.hrmpandjiadhi.ui.viewmodel.leave.LeaveTypesViewModel
import com.hrmpandjiadhi.ui.viewmodel.leave.LeaveTypesViewModelFactory
import java.io.File
...
class AddNewFormLeaveStaffActivity : AppCompatActivity() {

    private lateinit var binding: ActivityAddNewFormLeaveStaffBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        enableEdgeToEdge()
        binding = ActivityAddNewFormLeaveStaffBinding.inflate(layoutInflater)
    ...
    private fun setupListeners() {
        ...
        binding.powerSpinnerView2.setOnSpinnerItemSelectedListener<String> { _, _, _, item ->
            selectedItem = item
            typeId =
                typesLeaveViewModel.leaveTypes.value?.data?.find { it.type_name == item }?.id ?: 0
        }
        binding.powerSpinnerView2.setOnSpinnerOutsideTouchListener { _, _ ->
            binding.powerSpinnerView2.dismiss()
        }
    ...
    private fun setupViewModel() {
        ...
        createLeaveViewModel =
            ViewModelProvider(this, createLeaveViewModelFactory)[CreateLeaveViewModel::class.java]
    }
    ...
    private fun observeLeaveSubmission() {
        ...
        createLeaveViewModel.errorMessageLiveData.observe(this) { errorMessage ->
            if (!errorMessage.isNullOrEmpty()) {
                showLeaveDialog(false, "Error", errorMessage.toString())
                Toast.makeText(this, errorMessage.toString(), Toast.LENGTH_SHORT).show()
            }
        }
